...
- A before-and-after of what a crawler actually fetches. The claim that a
  Flutter web build hands a bot an empty document is easy to disbelieve,
  because the page looks fine in a browser. Both panels are read from files in
  the repository — the shell `flutter build web` writes and the page this tool
  writes — so the 0-words-to-52 is measured rather than drawn. Generated by
  `tool/crawler_view_figure.dart`.
